...
class Fatigue_detecting(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        # 设置窗口标题的图标
        self.icon = wx.Icon('./images/icon.ico', wx.BITMAP_TYPE_ICO)
        ...
        self.LONG_EYE_AR_CONSEC_FRAMES = 10
            ...
            for i in range(1):
                ...
                if args.save_image:
                    ...
                    for b in dets:
                                ...
                                if self.EyeClose >= self.LONG_EYE_AR_CONSEC_FRAMES:
                                    if self.long_EyeClose == 0:
                                        ...
                                        self.m_textCtrl3.AppendText(
                                            time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"闭眼\n")
                        ...
                        cv2.putText(img_raw, "Blinks: {}".format(self.blink_count), (450, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (255, 255, 0), 2)

                        if self.mask:
                            if self.blink_count >= 5 or self.long_EyeClose == 1:
                                cv2.putText(img_raw, "SLEEP!!!", (cx - 70, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 3)
                        else:
                    ...
                    if Face == False:
                        if self.face_checkBox4.GetValue() == True:
                            cv2.putText(img_raw, "No Face", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3,
                                        cv2.LINE_AA)
                            self.blink_count = 0
                            self.yawm_count = 0
                            self.EyeClose = 0
